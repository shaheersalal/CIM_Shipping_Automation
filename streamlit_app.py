...
with tab_setup:
    ...
    st.markdown("#### 📝 Setup Form")
    st.caption(
        "Per the client's requirement doc §4.1 -- \"a master setup form... capturing Port/location, "
        "Activity type, Free days, Trade person email, Head email, Director email.\" Only Port is "
        "required; everything else can be left blank."
    )

    with st.form("port_config_form", clear_on_submit=True):
        fc1, fc2, fc3 = st.columns(3)
        form_port = fc1.text_input("Port / Location")
        form_activity = fc2.text_input("Activity Type", help="Leave blank for a port-level default")
        form_free_days = fc3.number_input("Free Days", min_value=0, step=1, value=None)

        fc4, fc5, fc6 = st.columns(3)
        form_trade_email = fc4.text_input("Trade Person Email", help="Alert recipient for this port")
        form_head_email = fc5.text_input("Head Email")
        form_director_email = fc6.text_input("Director Email")

        submitted = st.form_submit_button("💾 Save this port")

    if submitted:
        if not form_port.strip():
            themed_alert("error", "Port / Location is required.")
        else:
            new_row = {
                "Port": form_port.strip(),
                "Activity Type": form_activity.strip() or None,
                "Free Days": form_free_days,
                "Trade Person Email": form_trade_email.strip() or None,
                "Head Email": form_head_email.strip() or None,
                "Director Email": form_director_email.strip() or None,
            }
